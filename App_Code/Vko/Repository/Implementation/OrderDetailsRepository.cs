...
( SELECT Id, MAX(seed) AS seed FROM (
    ...
    WHERE p.Id = od.ProductId AND s.ProductName LIKE :search
        ...
        public IEnumerable<T> Find<Y>(Y args)
        {
            ...
            {
                strSql = string.Format("SELECT od.* FROM OrderDetail od, {0} WHERE od.Id = res.Id ORDER BY res.seed DESC", strSqlSearch);
                return this.query.Run(strSql, new {
        ...
        public T Create(T orderDetail)
        {
            ...
            int rows = this.query.Insert(strSql, orderDetail);
            if (rows > 0)
            {
                object lastId = this.query.Scalar("SELECT last_insert_rowid()", new {});
        ...
        public T Update(object id, T orderDetail)
        {
            ...
            var res = this.query.Update(strSql, orderDetail, new {
                oid = id
            });
